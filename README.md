...
```js
import { ChestFormData } from './extensions/main.js';
```
...
## Inventory Section
The inventory section of the UI can be toggled by a boolean value in `<resource>/ui/_global_variables.json` and in `<behavior>/scripts/extensions/constants.js`. Make sure to disable or enable this for both.
...
1. Navigate to `<behavior>/scripts/extensions/constants.js`
2. Add the relevant item information into the `customContent` constant in the file!
